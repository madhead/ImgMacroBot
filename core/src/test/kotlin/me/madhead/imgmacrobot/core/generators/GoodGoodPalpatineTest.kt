...
internal class GoodGoodPalpatineTest {
    ...
    private val sut = GoodGoodPalpatine(templatesDir, imgur, cachedInlineQueryResultDAO, registry)

    @ParameterizedTest
    @MethodSource("parseInlineQueryParams")
    fun parseInlineQuery(inlineQuery: InlineQuery, parsedInlineQuery: GoodGoodPalpatineParsedInlineQuery?) {
    ...
    private fun parseInlineQueryParams(): List<Arguments> {
        ...
        return listOf(
            Arguments.of(
                BaseInlineQuery(id = "", from = user, query = "good", offset = ""),
                GoodGoodPalpatineParsedInlineQuery
            ),
            Arguments.of(
                BaseInlineQuery(id = "", from = user, query = "gooooooooood", offset = ""),
                GoodGoodPalpatineParsedInlineQuery
            ),
            Arguments.of(
                BaseInlineQuery(id = "", from = user, query = "good gooood", offset = ""),
                GoodGoodPalpatineParsedInlineQuery
            ),
            Arguments.of(
                BaseInlineQuery(id = "", from = user, query = "good, let the hate flow through you!", offset = ""),
                GoodGoodPalpatineParsedInlineQuery
            ),
            Arguments.of(
